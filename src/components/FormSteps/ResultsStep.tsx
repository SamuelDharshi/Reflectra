...
const ResultsStep: React.FC<ResultsStepProps> = ({ userData, onBack, onReset }) => {
  ...
  useEffect(() => {
    ...
    const saveToLocalStorage = (adviceText: string) => {
      ...
      localStorage.setItem('reflectraReflections', 
...
*Generated by Fallback Guidance System*`;
  ...
  const renderContentBlock = (block: string) => {
    ...
    const isBulletedList = lines.length > 1 && lines.every((line) => /^[-•]/.test(line));
  ...
  const downloadAsText = () => {
    ...
    const file = new Blob([
      `# Reflectra Reflection\n\nDate: ${new Date().toLocaleDateString()}\n\n${aiProvider ? `Generated by: ${aiProvider}` : 'Powered by AI'}\n\n${advice}\n\n---\n${quote.text}\n- ${quote.author}`
    ], { type: 'text/plain' });
    element.href = URL.createObjectURL(file);
    element.download = 'reflectra-reflection.txt';
              ...
              >
                ...
                {isFallback && (
                  <p className="text-sm text-amber-700 dark:text-amber-300">
                    Don't worry – we've created a personalized reflection below while the AI is
                    unavailable.
                  </p>
                )}
              </div>
